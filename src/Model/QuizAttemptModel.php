...
class QuizAttemptModel extends Database
{
    ...
    static function getQuizAttemptResult($quiz_attempt_id, $user_id)
    {
        $sql = "SELECT qa.id as attempt_id, quiz_id, qa.score, qa.created_at, qz.title, qz.description, qz.mode
        FROM `quiz_attempts` qa, quizzes as qz WHERE `qa`.`id` = :quiz_attempt_id AND qa.user_id = :user_id and qz.id = qa.quiz_id";
        return Database::query($sql, [':quiz_attempt_id' => $quiz_attempt_id, ':user_id' => $user_id])[0] ?? [];
    }
