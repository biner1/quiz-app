    ...
    <?php foreach ($quiz as $qu): ?>
      <li class="list-group-item">
        <div class="d-flex justify-content-between align-items-center">
          ...
          <span class="badge badge-primary badge-pill">Created by:
            <?php echo $qu['user_id']; ?>
          </span>
        </div>
        <p class="mt-2">Created at:
          <?php echo $qu['created_at']; ?>
        </p>
      </li>
    <?php endforeach; ?>
</ul>
...
<ol class="list-group">
  <?php foreach ($questions as $quiz_id => $quiz_data) { ?>
    <?php foreach ($quiz_data['questions'] as $question_text => $question_data) { ?>
      ...
      <li><input type="text" class="form-control" id="question_text" name="question_text"
          value="<?= $question_data['question_text']; ?>"></li>
      ...
      <div>
        ...
        <a class="btn btn-danger btn-sm delete-question-link"
          href="question/delete?qid=<?= $_GET['id']; ?>&id=<?= $question_data['question_id']; ?>">Delete Question</a>
      </div>
      </div>
      ...
      <ol type="A">
        <?php foreach ($question_data['options'] as $option_data) { ?>
          ...
          <li><input type="text" class="form-control" id="option_text" name="option_text"
              value="<?php echo $option_data['option_text']; ?>"></li>
          <input type="checkbox" class="form-check-input" name="is_correct"
            id="is_correct_<?php echo $option_data['option_id']; ?>" value="1" <?php if ($option_data['is_correct'])
                 echo 'checked'; ?>>
          ...
          <div>
            <a class="btn btn-danger btn-sm delete-option-link"
              href="option/delete?qid=<?= $_GET['id']; ?>&id=<?php echo $option_data['option_id']; ?>">Delete Option</a>
          </div>
